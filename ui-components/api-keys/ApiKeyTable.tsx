            ...
            <TableRow key={key.id} className="dark:border-zinc-700">
              <TableCell className="font-medium text-zinc-900 dark:text-zinc-100">
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  {key.graphId && (
                    <span
                      className="max-w-40 shrink-0 truncate rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-medium text-blue-800 dark:bg-blue-900 dark:text-blue-300"
                      title={`Scoped to graph ${key.graphId} — valid only for that graph and its subgraphs`}
                    >
                      {key.graphId}
                    </span>
                  )}
